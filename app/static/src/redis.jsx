...
const Redis = React.createClass({
  ...
  getInitialState: function () {
    return {
      ...
      ops_chart: this.defaultOption(
        "Redis Command Per Second",
        "",
        ["OPS"],
        "Cmd count",
        ""
      ),
      mem_chart: this.defaultOption(
        "Redis Mem Usage",
        "",
      ...
      cpu_chart: this.defaultOption(
        "Redis Cpu Usage",
        "",
        ["cpu_user", "cpu_sys", "cpu_user_children", "cpu_sys_children"],
        "Cpu Usage",
        ""
      ),
    };
  ...
  get_role_header_text: function (is_master, role_master_data) {
    if (is_master) {
      return (
        "Redis role [ master ], has " + role_master_data.length + " slaves"
      );
    } else if (is_master === false) {
      return (
        "Redis role [ slave ], master is " +
        this.state.redis_monitor.master_host +
        ":" +
        this.state.redis_monitor.master_port
      );
    } else {
  ...
  get_role_master_dom: function (is_master, role_master_data) {
    if (is_master) {
      if (role_master_data.length)
        ...
        return (
          <tr className="role_master_data">
            <td colSpan="8" style={{ textAlign: "center" }}>
              There has no redis slave.
            </td>
          </tr>
        );
    }
  ...
  render: function () {
    ...
    return (
      <div>
        <h1>
          [
          <span className="redis_host_port">
            {this.state.redis.host}:{this.state.redis.port}
          </span>
        ...
        >
          <tbody>
            ...
            <tr>
              ...
              <td colSpan="2">
                {Utils.sec_2_hour(this.state.redis_monitor.uptime_in_seconds)} /{" "}
                {this.state.redis_monitor.uptime_in_days} Days
              </td>
            ...
            <tr className="th_group">
              <th colSpan="8" ref="role_header">
                {this.get_role_header_text(is_master, role_master_data)}
              </th>
            </tr>
            {is_master && (
                ...
                return (
                  <tr key={i} className="rb_td">
                    ...
                    <td colSpan="2">
                      <input
                        type="button"
                        onClick={this.flush_redis.bind(
                          this,
                          this.props.params.md5,
                          e.key.substring(2)
                        )}
                        value="Flush DB"
                      />
